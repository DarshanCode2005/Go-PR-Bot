"""Tests for gin-gonic/gin repo skill."""
...
def _plan() -> FixPlan:
    return FixPlan(
        ...
        repo=_REPO,
        files=["context.go"],
        steps=["Fix Context bind regression"],
        test_commands=["go test ./pkg/... -count=1"],
        acceptance_criteria=["Regression test passes"],
    )


def test_gin_skill_file_exists():
    path = _SKILLS_ROOT / "gin-gonic__gin" / "SKILL.md"
    assert path.is_file(), "gin skill must exist at skills/gin-gonic__gin/SKILL.md"


def test_load_skill_text_for_gin():
    text = load_skill_text(_REPO)
    assert "github.com/gin-gonic/gin" in text
    assert "tree.go" in text
...
def test_resolve_lint_commands_uses_gin_skill_override(tmp_path):
